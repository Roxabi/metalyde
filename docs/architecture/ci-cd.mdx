...
description: Pipeline stages, quality gates, caching strategy, git hooks, and deployment automation in Metalyde.
---

## Overview

Metalyde uses a **three-layer quality pipeline** to catch issues at progressively broader scopes: local git hooks enforce standards on every commit and push, GitHub Actions CI validates every pull request and branch push, and Vercel handles production deployment automatically on merge to `main`.
...
Commitlint runs as a Lefthook `commit-msg` hook. Messages that fail validation are rejected before the commit is created. See [Dev Process](../processes/dev-process) for full conventions including scope guidelines and breaking change format.
...
- [Dev Process](../processes/dev-process) -- Commit conventions and development workflow
