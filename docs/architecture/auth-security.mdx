...
Authentication and security form a layered defense across the Metalyde stack. The system is built on **Better Auth** — a library that handles session lifecycle, OAuth flows, email verification, and organization management — integrated into NestJS through a thin service wrapper and a global guard.
