---
title: Deployment
description: Deploy Metalyde on Vercel — both web and API on one platform
---

## Overview

Metalyde uses a **staging-based** deployment architecture with **Vercel** as the single platform for both the web app and API.
...
| Docs (Fumadocs + Next.js) | Vercel | Static/ISR — `docs.metalyde.roxabi.dev`, published from `main` only (no staging environment) |
...
- Web and API have a standing **staging** preview environment (`staging.metalyde.roxabi.dev`); the **docs** site has no staging environment — docs changes are reviewed on `main` after promotion
...
By default, preview deploys compute `APP_URL` from the Vercel branch alias (e.g., `metalyde-app-git-staging-roxabi.vercel.app`). Branches with a custom domain alias (e.g., `staging.metalyde.roxabi.dev`) need `APP_URL` set to the custom domain so better-auth's CSRF origin check passes.

The workflow handles this automatically for known branches:

| Branch | `APP_URL` | `CORS_ORIGIN` |
|--------|-----------|---------------|
| `staging` | `https://staging.metalyde.roxabi.dev` | `https://staging.metalyde.roxabi.dev,https://metalyde-app-git-staging-roxabi.vercel.app` |
...
for env in production preview development; do
  printf '%s' "https://api.metalyde.vercel.app" | vercel env add API_URL "$env"
  printf '%s' "https://app.metalyde.vercel.app" | vercel env add APP_URL "$env"
done
```
...
for env in production preview development; do
  printf '%s' "https://app.metalyde.vercel.app" | vercel env add CORS_ORIGIN "$env"
  printf '%s' "$SECRET"                       | vercel env add BETTER_AUTH_SECRET "$env"
  printf '%s' "https://api.metalyde.vercel.app" | vercel env add BETTER_AUTH_URL "$env"
  printf '%s' "https://app.metalyde.vercel.app" | vercel env add APP_URL "$env"
  printf '%s' "https://api.metalyde.vercel.app" | vercel env add API_URL "$env"
...
```bash
cd apps/api
vercel integration add upstash/upstash-kv --name metalyde-redis
```
...
If you are forking Metalyde to start your own SaaS project, follow this step-by-step checklist to get a fully working CI/CD pipeline.
...
This requires **Vercel team SSO authentication** to access any deployment URL that is not a custom domain. The `staging` branch has custom domains (`staging.metalyde.roxabi.dev`, `api.staging.roxabi.com`) which bypass SSO. Other preview deployments use Vercel-generated URLs and are gated behind SSO login.
