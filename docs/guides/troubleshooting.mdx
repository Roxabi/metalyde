...
description: Common issues and solutions for Metalyde development
---
...
If you copied `.env.example` but changed nothing, verify the default credentials (`metalyde:metalyde`) match what `docker-compose.yml` defines. A common mistake is editing the password in one place but not the other:

```bash
# Verify by connecting manually (replace metalyde-postgres if POSTGRES_CONTAINER is overridden)
docker exec -it metalyde-postgres psql -U metalyde -d metalyde -c "SELECT 1"
```
...
**Symptom:** After creating a git worktree, the API fails to start with a database error like `database "metalyde_<number>" does not exist` or migrations fail.

**Cause:** Each worktree is intended to use its own isolated branch database (`metalyde_<issue_number>`). The `db:branch:create` script creates this database, runs migrations, seeds it, and updates the worktree's `.env` with the correct `DATABASE_URL`. If you skipped this step, the worktree's `.env` still points to the default `metalyde` database (or worse, no database at all).

**Fix:**

**Important:** Run this from inside your worktree directory (e.g., `../metalyde-42/`), not from the main repository checkout. Running from the wrong directory will update the wrong `.env` file.
...
This will:
- Create the `metalyde_<issue_number>` database in the Docker container
- Run all migrations
- Seed the database with dev data (`dev@metalyde.local` / `password123`)
...
For more on branch databases, see [Developer Environment](./dev-environment).

## After Pulling Admin Panel Changes (PR #279)

If you had an existing Docker Postgres setup before this PR, the `metalyde_app` database user
...
This creates the `metalyde_app` user with RLS-enforced permissions. Without it, tenant-scoped
queries will fail with "permission denied to set role app_user".

Alternative: delete your Docker volume and recreate: `bun run db:down && docker volume rm metalyde_postgres_data && bun run db:up`

> **Note:** If `POSTGRES_VOLUME` is set to a custom value (e.g. `myapp_data`), the Docker volume name will be `metalyde_myapp_data` — adjust the `docker volume rm` command accordingly.
...
For commit message rejections, use the Conventional Commits format: `type(scope): description` (e.g., `feat(web): add login page`).
...
**Cause:** `docker-compose.yml` defaults to fixed container names (`metalyde-postgres`, `metalyde-mailpit`) and a fixed volume name (`postgres_data`). Two projects sharing these names on the same Docker daemon will conflict.
...
Set these in `.env` before running `bun db:up` to avoid naming conflicts with sibling projects.
