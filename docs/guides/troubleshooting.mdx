...
> **Note:** If `POSTGRES_VOLUME` is set to a custom value (e.g. `myapp_data`), the Docker volume name will be `myapp_data` — adjust the `docker volume rm` command accordingly.
