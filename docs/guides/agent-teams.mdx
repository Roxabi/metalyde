...
|-------|-------|--------|-----------|
| **frontend-dev** | white | `apps/web/`, `packages/ui/` | Frontend Patterns |
| **backend-dev** | white | `apps/api/`, `packages/types/` | Backend Patterns |
| **devops** | white | `packages/config/`, root configs | — |

### Quality Agents (verification and fixing)

| Agent | Color | Role | Standards |
|-------|-------|------|-----------|
| **fixer** | white | Fullstack quick fixer — applies accepted review comments. Spawned in parallel (one per domain) when fixes span multiple domains. | Code Review |
| **tester** | white | Test generation &amp; validation | Testing |
...
| **doc-writer** | white | Documentation maintenance | — |
