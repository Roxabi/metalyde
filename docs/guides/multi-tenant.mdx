...
description: How tenant isolation works in Metalyde -- from the request interceptor through CLS context to PostgreSQL Row-Level Security.
---

## Overview

Metalyde uses **PostgreSQL Row-Level Security (RLS)** for tenant data isolation. Every organization is a tenant, and business data is scoped by a `tenant_id` column that maps to `organizations.id`.
