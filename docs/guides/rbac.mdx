...
Metalyde uses a **flat, per-organization RBAC model**. Each organization (tenant) has its own set of roles, and each role is an explicit list of `resource:action` permission strings. There is no role inheritance -- every role declares exactly what it can do.
